...
public class GuiTextArea extends Gui {
    ...
    public void drawTextBox() {
        ...
        for (int i = 0; i < lineCount; i++) {
            fontRenderer
                    .drawStringWithShadow(text[i], textLeft, textTop + (fontRenderer.FONT_HEIGHT + 1) * i, textColor);
        }
    ...
    public void setCursorPosition(int x, int y) {
        ...
        if (this.cursorPosition > lineLength) this.cursorPosition = lineLength;
    }
    ...
    public void deleteFromCursor(int count) {
        if (text[cursorLine].length() != 0) {
            ...
            if (left >= 0) newLine = curLine.substring(0, left);

            if (right < curLine.length()) newLine = newLine + curLine.substring(right);

            text[cursorLine] = newLine;

            if (back) setCursorPosition(cursorPosition + count, cursorLine);
        }
    }
