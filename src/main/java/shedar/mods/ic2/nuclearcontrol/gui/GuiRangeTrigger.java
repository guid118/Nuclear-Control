...
public class GuiRangeTrigger extends GuiContainer {
    ...
    private void initControls() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            buttonList.add(
                    new CompactButton(i * 10 + 1, guiLeft + 30 + i * 12 + (i + 2) / 3 * 6, guiTop + 42, 12, 12, "+"));
        }
        for (int i = 0; i < 10; i++) {
            buttonList.add(
    ...
    protected void actionPerformed(GuiButton button) {
        ...
        if (newValue != initValue) {
            ...
            if (isEnd) trigger.setLevelEnd(newValue);
            else trigger.setLevelStart(newValue);
        }
    }
