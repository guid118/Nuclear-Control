...
public class BlockNuclearControlMain extends BlockContainer {
    ...
    public int onBlockPlaced(World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ,
        ...
        if (isSolidBlockRequired(metadata)
                && !world.isSideSolid(x + opposite.offsetX, y + opposite.offsetY, z + opposite.offsetZ, dir)) {
            side = 1;
        }
    ...
    /*
     * @Override public void onBlockDestroyedByPlayer(World world, int x, int y, int z, int metadata) { TileEntity
     * tileEntity = world.getTileEntity(x, y, z); if (tileEntity instanceof TileEntityHowlerAlarm) {
     * ((TileEntityHowlerAlarm) tileEntity).setPowered(false); } if (!world.isRemote && tileEntity instanceof
     * IInventory) { IInventory inventory = (IInventory) tileEntity; float range = 0.7F; for (int i = 0; i <
     * inventory.getSizeInventory(); i++) { ItemStack itemStack = inventory.getStackInSlot(i); if (itemStack != null) {
     * double dx = world.rand.nextFloat() * range + (1.0F - range) * 0.5D; double dy = world.rand.nextFloat() * range +
     * (1.0F - range) * 0.5D; double dz = world.rand.nextFloat() * range + (1.0F - range) * 0.5D; EntityItem item = new
     * EntityItem(world, x + dx, y + dy, z + dz, itemStack); item.delayBeforeCanPickup = 10;
     * world.spawnEntityInWorld(item); inventory.setInventorySlotContents(i, null); } } } super.breakBlock(world, x, y,
     * z, this, metadata); }
     */
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block neighbor) {
        ...
        if (isSolidBlockRequired(metadata) && !world.isSideSolid(
                x + Facing.offsetsXForSide[side],
                y + Facing.offsetsYForSide[side],
                z + Facing.offsetsZForSide[side],
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess blockAccess, int x, int y, int z) {
        ...
        setBlockBounds(
                ...
                Math.min(baseZ1, baseZ2),
                Math.max(baseX1, baseX2),
                Math.max(baseY1, baseY2),
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float f1, float f2,
        ...
        if (subblocks.containsKey(blockType) && subblocks.get(blockType).hasGui()) {
            if (player instanceof EntityPlayerMP) {
                ...
                PlaceEvent e = ForgeEventFactory
                        .onPlayerBlockPlace(player, blockSnapshot, ForgeDirection.getOrientation(side));
                if (!e.isCanceled()) {
    ...
    public int isProvidingStrongPower(IBlockAccess iblockaccess, int x, int y, int z, int direction) {
        ...
        if (!(tileentity instanceof TileEntityThermo) && !(tileentity instanceof TileEntityRangeTrigger)) return 0;
        ...
        if (tileentity instanceof TileEntityThermo && targetEntity != null
                && (NuclearHelper.getReactorAt(tileentity.getWorldObj(), targetX, targetY, targetZ) != null
                        || NuclearHelper.getReactorChamberAt(tileentity.getWorldObj(), targetX, targetY, targetZ)
                                != null)) {
            return 0;
        }
        ...
        if (tileentity instanceof TileEntityRangeTrigger) return ((TileEntityRangeTrigger) tileentity).getOnFire() > 0
                ^ ((TileEntityRangeTrigger) tileentity).isInvertRedstone() ? 15 : 0;
        return ((TileEntityThermo) tileentity).getOnFire() > 0 ^ ((TileEntityThermo) tileentity).isInvertRedstone() ? 15
                : 0;
    }

    @Override
    public IIcon getIcon(int side, int metadata) {
        if (subblocks.containsKey(metadata)) return subblocks.get(metadata).getBlockTextureFromSide(side);
        return null;
    }
    ...
    public TileEntity createTileEntity(World world, int metadata) {
        if (subblocks.containsKey(metadata)) return subblocks.get(metadata).getTileEntity();
        return null;
    }
    ...
    public int colorMultiplier(IBlockAccess world, int x, int y, int z) {
        if (world.getBlock(x, y, z).equals(this) && world.getBlockMetadata(x, y, z) == BlockDamages.DAMAGE_HOWLER_ALARM
                && world.getTileEntity(x, y, z) instanceof TileEntityHowlerAlarm) {
