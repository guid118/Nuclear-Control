...
public class ItemKitReactorSensor extends ItemSensorKitBase {
    ...
    protected ItemStack getItemStackByDamage(int cardID) {
        if (cardID == 0) return new ItemStack(IC2NuclearControl.itemSensorLocationCard, 1, 0);
        if (cardID == 1) return new ItemStack(IC2NuclearControl.item55ReactorCard, 1, 0);
        else return null;
    }
    ...
    public boolean onItemUseFirst(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (position != null) {
            ...
            if (check == Block.getBlockFromItem(IC2Items.getItem("reactorRedstonePort").getItem())
                    ...
                    || check == Block.getBlockFromItem(Ic2Items.reactorAccessHatch.getItem())
                    || check == Block.getBlockFromItem(Ic2Items.reactorFluidPort.getItem())) {
                sendInt = 1;
                ItemStack sensorLocationCard = getItemStackByDamage(sendInt);
