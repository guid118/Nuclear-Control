...
public class ItemNuclearControlMain extends ItemBlock {
    ...
    public boolean placeBlockAt(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (world.getBlock(x, y, z) == field_150939_a) {
            ...
            else field_150939_a.onBlockPlacedBy(world, x, y, z, player, stack);
    ...
    public boolean func_150936_a(World world, int x, int y, int z, int side, EntityPlayer player, ItemStack item) {
        ...
        /*
         * if((Math.floor(player.posX) == x || Math.ceil(player.posX) == x) && (Math.floor(player.posZ) == y ||
         * Math.ceil(player.posZ) == y)) { if((Math.floor(player.posY) + 2 == y || Math.ceil(player.posY) + 2 == y) ||
         * (Math.floor(player.posY) + 2 == y || Math.ceil(player.posY) + 2 == y)){ return false; } }
         */
